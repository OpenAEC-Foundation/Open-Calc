  ...
  return (
    <Card className="bg-white p-8 max-w-4xl mx-auto shadow-lg print:shadow-none print:p-0">
      ...
      {(estimate.offerWorkDescription ||
        ...
        <div className="mt-8 print:break-before-page">
          <h2 className="text-xl font-bold text-primary mb-2">Offertespecificatie</h2>
          <p className="text-sm text-muted-foreground mb-6">
            Behorend bij offerte {estimate.project.projectNumber || estimate.version}
          </p>
          <Separator className="mb-6" />

          <p className="text-sm text-muted-foreground mb-6">
            Tot de genoemde offerte behoren de hieronder omschreven werkzaamheden en toe te passen materialen.
          </p>

          {/* Werkzaamheden */}
          {estimate.offerWorkDescription && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">De uit te voeren werkzaamheden bestaan uit</h3>
              <ul className="text-sm space-y-1 ml-4">
                {estimate.offerWorkDescription.split("\n").filter(line => line.trim()).map((line, i) => (
          ...
          {estimate.offerMaterials && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Ten tijde van de montage worden de volgende materialen toegepast</h3>
              <ul className="text-sm space-y-1 ml-4">
                {estimate.offerMaterials.split("\n").filter(line => line.trim()).map((line, i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-muted-foreground">•</span>
                    <span>{line.trim()}</span>
                  </li>
          ...
          {estimate.offerEquipment && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Ten tijde van de montage wordt het volgende materieel toegepast</h3>
              <ul className="text-sm space-y-1 ml-4">
                {estimate.offerEquipment.split("\n").filter(line => line.trim()).map((line, i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-muted-foreground">•</span>
                    <span>{line.trim()}</span>
                  </li>
          ...
          {estimate.offerExclusions && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">De offerte is exclusief</h3>
              <ul className="text-sm space-y-1 ml-4">
                {estimate.offerExclusions.split("\n").filter(line => line.trim()).map((line, i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-muted-foreground">•</span>
                    <span>{line.trim()}</span>
                  </li>
          ...
          {/* Meerwerk */}
          {(estimate.offerExtraWorkTerms || estimate.offerExtraWorkRate) && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Meerwerk en aanvullende kosten</h3>
              {estimate.offerExtraWorkTerms && (
                <p className="text-sm mb-2 ml-4">{estimate.offerExtraWorkTerms}</p>
              )}
              {estimate.offerExtraWorkRate && (
                <p className="text-sm ml-4">
                  Uurtarief meerwerk: {formatCurrency(estimate.offerExtraWorkRate)} (exclusief BTW)
                </p>
              )}
            </div>
          )}
          ...
          {estimate.offerPaymentTerms && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Betalingsvoorwaarden</h3>
              <ul className="text-sm space-y-1 ml-4">
                {estimate.offerPaymentTerms.split("\n").filter(line => line.trim()).map((line, i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-muted-foreground">•</span>
                    <span>{line.trim()}</span>
                  </li>
          ...
          {/* Planning */}
          {(estimate.offerValidityWeeks || estimate.offerPlanningNotes) && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Planning</h3>
              <ul className="text-sm space-y-1 ml-4">
                <li className="flex gap-2">
                  <span className="text-muted-foreground">•</span>
                  <span>Geldigheid offerte betreft {estimate.offerValidityWeeks} weken</span>
                </li>
                {estimate.offerPlanningNotes && estimate.offerPlanningNotes.split("\n").filter(line => line.trim()).map((line, i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-muted-foreground">•</span>
                    <span>{line.trim()}</span>
                  </li>
