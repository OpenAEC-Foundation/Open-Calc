...
async function main() {
  const projectId = "cmkzd34eg0001yfh0q3c11ndz";

  // Delete old estimate if exists
  await prisma.estimateLine.deleteMany({ where: { estimate: { projectId } } });
  await prisma.estimateChapter.deleteMany({ where: { estimate: { projectId } } });
  ...
  const estimate = await prisma.estimate.create({
    data: {
      ...
      description: "Begroting op basis van opname 23 januari 2026 - inclusief optionele aanbouw",
      version: 1,
      status: "DRAFT",
  ...
  async function createChapter(code, name, sortOrder) {
  ...
  async function createLine(chapterId, description, options = {}) {
  ...
  await createLine(ch00.id, "Container huur (afvoer puin)", { quantity: 6, unit: "wk", materialCost: 350, specification: "10m3 container" });
  ...
  await createLine(ch46.id, "Vloerafwerking aanbouw achterzijde", { quantity: 25, unit: "m2", laborHours: 1.0, materialCost: 85, specification: "Nog te bepalen" });
  ...
  await createLine(ch56.id, "Vloerverwarming aanbouw achterzijde", { quantity: 25, unit: "m2", laborHours: 0.8, materialCost: 65 });
  ...
  await createLine(ch61.id, "Elektra aanbouw achterzijde", { quantity: 1, unit: "ps", laborHours: 16, materialCost: 400 });
  ...
  // 90 - Diversen
  const ch90 = await createChapter("90", "Diversen en stelposten", 18);
  ...
  // ===== OPTIONELE AANBOUW NAAST WONING =====
  const ch95 = await createChapter("95", "OPTIONEEL: Aanbouw naast woning (ca. 6,5 x 5m)", 19);

  // Fundering en constructie
  await createLine(ch95.id, "Bouwkundige tekeningen en vergunning", { quantity: 1, unit: "ps", subcontrCost: 3500, specification: "Architect/tekenbureau + leges" });
  await createLine(ch95.id, "Constructieberekeningen aanbouw", { quantity: 1, unit: "ps", subcontrCost: 1200 });
  await createLine(ch95.id, "Grondwerk en fundering (stroken/poeren)", { quantity: 32.5, unit: "m2", laborHours: 2, materialCost: 120, specification: "Ca. 6,5 x 5m" });
  await createLine(ch95.id, "Betonvloer met isolatie", { quantity: 32.5, unit: "m2", laborHours: 1.5, materialCost: 95, specification: "Gewapend beton + XPS isolatie" });

  // Wanden en dak
  await createLine(ch95.id, "Buitenwanden houtskelet of steenachtig", { quantity: 46, unit: "m2", laborHours: 1.8, materialCost: 145, specification: "Ca. 23m1 x 2m hoog" });
  await createLine(ch95.id, "Binnenwanden (scheidingswanden)", { quantity: 24, unit: "m2", laborHours: 1.5, materialCost: 65, specification: "Slaapkamer, inloopkast, douche" });
  await createLine(ch95.id, "Plat dak constructie met isolatie", { quantity: 32.5, unit: "m2", laborHours: 2, materialCost: 165, specification: "EPDM of bitumen, Rc 6.0" });

  // Kozijnen en deuren
  await createLine(ch95.id, "Aluminium kozijnen aanbouw", { quantity: 3, unit: "st", laborHours: 4, materialCost: 1400, specification: "HR++ inclusief montage" });
  await createLine(ch95.id, "Buitendeur naar tuin", { quantity: 1, unit: "st", laborHours: 4, materialCost: 1200 });
  await createLine(ch95.id, "Binnendeur vanuit bijkeuken", { quantity: 1, unit: "st", laborHours: 4, materialCost: 550 });
  await createLine(ch95.id, "Binnendeuren slaapkamer/inloopkast", { quantity: 2, unit: "st", laborHours: 3, materialCost: 420 });

  // Afwerking
  await createLine(ch95.id, "Stucwerk wanden aanbouw", { quantity: 70, unit: "m2", laborHours: 0.5, materialCost: 8 });
  await createLine(ch95.id, "Plafondafwerking aanbouw", { quantity: 32.5, unit: "m2", laborHours: 0.8, materialCost: 35 });
  await createLine(ch95.id, "Vloerafwerking slaapkamer/inloopkast", { quantity: 26, unit: "m2", laborHours: 1.0, materialCost: 75, specification: "Laminaat of PVC" });
  await createLine(ch95.id, "Tegelwerk doucheruimte", { quantity: 12, unit: "m2", laborHours: 2.5, materialCost: 85, specification: "Wand en vloer" });

  // Installaties aanbouw
  await createLine(ch95.id, "Elektra-installatie aanbouw compleet", { quantity: 1, unit: "ps", laborHours: 24, materialCost: 650, specification: "Incl. spots, schakelaars, stopcontacten" });
  await createLine(ch95.id, "Waterleiding naar doucheruimte", { quantity: 1, unit: "ps", laborHours: 8, materialCost: 350 });
  await createLine(ch95.id, "Riolering doucheruimte", { quantity: 1, unit: "ps", laborHours: 8, materialCost: 280 });
  await createLine(ch95.id, "Vloerverwarming aanbouw", { quantity: 32.5, unit: "m2", laborHours: 0.8, materialCost: 65 });
  await createLine(ch95.id, "Airco binnenunit slaapkamer aanbouw", { quantity: 1, unit: "st", laborHours: 4, materialCost: 1200 });

  // Sanitair aanbouw
  await createLine(ch95.id, "Inloopdouche compleet", { quantity: 1, unit: "st", laborHours: 8, materialCost: 1800, specification: "Incl. douchewand, kraan, afvoer" });
  await createLine(ch95.id, "Wastafelmeubel", { quantity: 1, unit: "st", laborHours: 4, materialCost: 650 });
  await createLine(ch95.id, "Toilet (optioneel)", { quantity: 1, unit: "st", laborHours: 4, materialCost: 550, specification: "Indien gewenst" });

  // Ventilatie
  await createLine(ch95.id, "Mechanische ventilatie aanbouw", { quantity: 1, unit: "ps", laborHours: 8, materialCost: 850 });
  ...
  // Calculate optionele aanbouw apart
  const aanbouwChapter = chapters.find(c => c.code === "95");
  const aanbouwLines = await prisma.estimateLine.findMany({ where: { chapterId: aanbouwChapter.id } });
  const aanbouwTotaal = aanbouwLines.reduce((sum, l) => sum + l.totalPrice, 0);

  // Print summary
  console.log("\n========================================");
  console.log("BEGROTING AANGEMAAKT");
  console.log("========================================");
  console.log("Project: Renovatie Burgemeester Lepelaarssingel 38");
  console.log("Projectnummer: 2967");
  console.log("Adres: Burgemeester Lepelaarssingel 38, Krimpen aan den IJssel");
  console.log("----------------------------------------");
  console.log(`Begroting: ${estimate.name}`);
  console.log(`Aantal hoofdstukken: ${chapters.length}`);
  console.log(`Aantal regels: ${lines.length}`);
  console.log("----------------------------------------");
  console.log(`Arbeid:         € ${totals.totalLabor.toFixed(2).padStart(12)}`);
  console.log(`Materiaal:      € ${totals.totalMaterial.toFixed(2).padStart(12)}`);
  console.log(`Materieel:      € ${totals.totalEquipment.toFixed(2).padStart(12)}`);
  console.log(`Onderaanneming: € ${totals.totalSubcontr.toFixed(2).padStart(12)}`);
  console.log("----------------------------------------");
  console.log(`Subtotaal:      € ${subtotal.toFixed(2).padStart(12)}`);
  console.log(`AK (${estimate.generalCostsPercent}%):       € ${generalCostsAmount.toFixed(2).padStart(12)}`);
  console.log(`Winst (${estimate.profitPercent}%):    € ${profitAmount.toFixed(2).padStart(12)}`);
  console.log(`Risico (${estimate.riskPercent}%):   € ${riskAmount.toFixed(2).padStart(12)}`);
  console.log("----------------------------------------");
  console.log(`Totaal excl:    € ${totalExclVat.toFixed(2).padStart(12)}`);
  console.log(`BTW (${estimate.vatPercent}%):      € ${vatAmount.toFixed(2).padStart(12)}`);
  console.log("========================================");
  console.log(`TOTAAL INCL:    € ${totalInclVat.toFixed(2).padStart(12)}`);
  console.log("========================================");
  console.log("");
  console.log("Waarvan OPTIONELE AANBOUW (hoofdstuk 95):");
  console.log(`  Subtotaal:    € ${aanbouwTotaal.toFixed(2).padStart(12)}`);
  console.log(`  Incl. opslag: € ${(aanbouwTotaal * 1.2 * 1.21).toFixed(2).padStart(12)} (incl. AK/W&R/BTW)`);
  console.log("========================================");
  console.log(`\nOpen: http://localhost:3002/projects/${projectId}/estimates/${estimate.id}`);
}
