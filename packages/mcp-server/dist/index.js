...
async function updateEstimateTotals(estimateId) {
...
async function updateChapterTotals(chapterId) {
    const lines = await prisma.estimateLine.findMany({
        where: { chapterId },
    });
    const totals = lines.reduce((acc, line) => ({
        ...
        switch (name) {
            ...
            case "create_estimate": {
                ...
                const estimate = await prisma.estimate.create({
                    data: {
                        ...
                        version: nextVersion,
                        status: "DRAFT",
                        generalCostsPercent: args.generalCostsPercent ?? 10,
                        profitPercent: args.profitPercent ?? 5,
                        riskPercent: args.riskPercent ?? 3,
                        vatPercent: args.vatPercent ?? 21,
            ...
            case "create_estimate_line": {
                ...
                const whereClause = args.chapterId
                    ...
                    : { estimateId: args.estimateId, chapterId: null };
                ...
                const laborRate = args.laborRate || 45;
                const laborCost = laborHours * laborRate;
                const materialCost = args.materialCost || 0;
                const equipmentCost = args.equipmentCost || 0;
                const subcontrCost = args.subcontrCost || 0;
                ...
                // Update totals
                await updateEstimateTotals(args.estimateId);
                if (args.chapterId) {
                    await updateChapterTotals(args.chapterId);
                }
                return {
                    content: [
                        {
            ...
            case "update_estimate_line": {
                ...
                const laborHours = args.laborHours ?? line.laborHours ?? 0;
                const laborRate = args.laborRate ?? line.laborRate ?? 45;
                const laborCost = laborHours * laborRate;
                const materialCost = args.materialCost ?? line.materialCost ?? 0;
                const equipmentCost = args.equipmentCost ?? line.equipmentCost ?? 0;
                const subcontrCost = args.subcontrCost ?? line.subcontrCost ?? 0;
                const unitPrice = laborCost + materialCost + equipmentCost + subcontrCost;
                const quantity = args.quantity ?? line.quantity ?? 1;
                const totalPrice = unitPrice * quantity;
                const updatedLine = await prisma.estimateLine.update({
                    where: { id: args.lineId },
                    data: {
                        code: args.code !== undefined ? args.code : undefined,
                        description: args.description !== undefined ? args.description : undefined,
                        specification: args.specification !== undefined ? args.specification : undefined,
                        quantity,
                        unit: args.unit !== undefined ? args.unit : undefined,
                        laborHours,
                        laborRate,
                        laborCost,
                        materialCost,
                        ...
                        AND: [
                            { isActive: true },
                            {
                                OR: [
                                    ...
                                    { specification: { contains: args.query } },
                                ],
                            },
            ...
            case "update_estimate_settings": {
                const estimate = await prisma.estimate.update({
                    ...
                    data: {
                        ...
                        description: args.description !== undefined ? args.description : undefined,
                        generalCostsPercent: args.generalCostsPercent !== undefined ? args.generalCostsPercent : undefined,
                        profitPercent: args.profitPercent !== undefined ? args.profitPercent : undefined,
                        riskPercent: args.riskPercent !== undefined ? args.riskPercent : undefined,
                        vatPercent: args.vatPercent !== undefined ? args.vatPercent : undefined,
            ...
            case "duplicate_estimate": {
                ...
                const targetProjectId = args.targetProjectId || source.projectId;
                // Get next version
                const existingEstimates = await prisma.estimate.findMany({
                ...
                const newEstimate = await prisma.estimate.create({
                    data: {
                        name: args.newName || `${source.name} (kopie)`,
